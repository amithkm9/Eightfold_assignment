...
| **Determinism guarantee** | Input-hash **caching** | `temperature=0` | Re-runs read the cache and never re-hit the model, so output is byte-identical regardless of any sampling setting — caching, not `temperature=0`, is what guarantees it. |
